...
#        print(self.z)
    ...
    def setParams(self, params):
        ...
        W3_end = W1_end + W2_end + self.hiddenLayerSize*self.outputLayerSize
...
zeus = df[df['heroes']=='Zeus'].copy()

for col in  ["kills", "assists", "deaths"]:
    zeus[col] = zeus[col]/zeus['duration']
...
cols = ['gold_per_min', 'xp_per_min', "kills", "assists", "deaths", 'duration', 'kda']
inSize = len(cols)
...
#NN.forward(x_train)
#print(NN.costFunction(x_train, y_train))
#T = trainer(NN)
#T.train(x_train, y_train)
#cost = NN.costFunction(x_test, y_test)
...
X = np.identity(inSize)
#np.random.seed(1)
costs_train = []
costs_test = []
#yHatFull = np.array([1,1,1,1,1,1]).reshape(6,1)
a = 0.005
Nit = 5000
hidSize = 20
NN = Neural_network(inSize, hidSize)
yHatFull = NN.forward(X)
for i in range(Nit):
    yHat = NN.forward(X)
    yHatFull = np.append(yHatFull, yHat, axis=1)
    costs_train.append(NN.costFunction(x_train, y_train))
    costs_test.append(NN.costFunction(x_test, y_test))
    dJW1, dJW2, dJW3 = NN.costFunctionPrime(x_train, y_train)
    NN.W1 = NN.W1 - dJW1*a
    NN.W2 = NN.W2 - dJW2*a
    NN.W3 = NN.W3 - dJW3*a
...
#    plt.plot(list(range(Nit+1)), yHatFull[i, :])
#plt.legend(cols)
##NN = Neural_network(inSize, hidSize)
#
yHat = NN.forward(X)
